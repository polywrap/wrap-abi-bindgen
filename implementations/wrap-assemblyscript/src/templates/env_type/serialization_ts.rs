lazy_static! {
  static ref NAME: String = "env_type/serialization.ts".to_string();
...
export function serialize{{type}}(type: {{detect_keyword type}}): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) env-type: {{type}}");
  const sizer = new WriteSizer(sizerContext);
  write{{type}}(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) env-type: {{type}}");
...
export function deserialize{{type}}(buffer: ArrayBuffer): {{detect_keyword type}} {
  const context: Context = new Context("Deserializing env-type {{type}}");
