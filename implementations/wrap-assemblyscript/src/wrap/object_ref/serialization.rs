...
pub fn serialize_object_ref(args: &ObjectRef) -> Result<Vec<u8>, EncodeError> {
    let mut encoder_context = Context::new();
    encoder_context.description = "Serializing (encoding) object-type: ObjectRef".to_string();
    let mut encoder = WriteEncoder::new(&[], encoder_context);
    write_object_ref(args, &mut encoder)?;
    Ok(encoder.get_buffer())
}

pub fn write_object_ref<W: Write>(args: &ObjectRef, writer: &mut W) -> Result<(), EncodeError> {
...
pub fn deserialize_object_ref(args: &[u8]) -> Result<ObjectRef, DecodeError> {
    let mut context = Context::new();
    context.description = "Deserializing object-type: ObjectRef".to_string();
    let mut reader = ReadDecoder::new(args, context);
    read_object_ref(&mut reader)
}

pub fn read_object_ref<R: Read>(reader: &mut R) -> Result<ObjectRef, DecodeError> {
