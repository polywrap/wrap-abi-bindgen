...
pub fn serialize_enum_ref(args: &EnumRef) -> Result<Vec<u8>, EncodeError> {
    let mut encoder_context = Context::new();
    encoder_context.description = "Serializing (encoding) object-type: EnumRef".to_string();
    let mut encoder = WriteEncoder::new(&[], encoder_context);
    write_enum_ref(args, &mut encoder)?;
    Ok(encoder.get_buffer())
}

pub fn write_enum_ref<W: Write>(args: &EnumRef, writer: &mut W) -> Result<(), EncodeError> {
...
pub fn deserialize_enum_ref(args: &[u8]) -> Result<EnumRef, DecodeError> {
    let mut context = Context::new();
    context.description = "Deserializing object-type: EnumRef".to_string();
    let mut reader = ReadDecoder::new(args, context);
    read_enum_ref(&mut reader)
}

pub fn read_enum_ref<R: Read>(reader: &mut R) -> Result<EnumRef, DecodeError> {
