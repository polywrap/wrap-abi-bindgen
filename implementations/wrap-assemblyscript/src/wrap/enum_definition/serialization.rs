...
pub fn serialize_enum_definition(args: &EnumDefinition) -> Result<Vec<u8>, EncodeError> {
    let mut encoder_context = Context::new();
    encoder_context.description = "Serializing (encoding) object-type: EnumDefinition".to_string();
    let mut encoder = WriteEncoder::new(&[], encoder_context);
    write_enum_definition(args, &mut encoder)?;
    Ok(encoder.get_buffer())
}

pub fn write_enum_definition<W: Write>(args: &EnumDefinition, writer: &mut W) -> Result<(), EncodeError> {
...
pub fn deserialize_enum_definition(args: &[u8]) -> Result<EnumDefinition, DecodeError> {
    let mut context = Context::new();
    context.description = "Deserializing object-type: EnumDefinition".to_string();
    let mut reader = ReadDecoder::new(args, context);
    read_enum_definition(&mut reader)
}

pub fn read_enum_definition<R: Read>(reader: &mut R) -> Result<EnumDefinition, DecodeError> {
